...
const Home = () => {
  ...
  const ALL_TIME_ARTISTS_ENDPOINT =
  ...
  const ALL_TIME_TRACKS_ENDPOINT =
    "https://api.spotify.com/v1/me/top/tracks?time_range=long_term&offset=0&limit=10";
  ...
  const downloadImg = () => {
    ...
    domtoimage
      .toJpeg(container, { quality, backgroundColor })
      .then((dataUrl) => {
  ...
  return (
    <div className="whole-page">
      ...
      <button className="button" id="topA" onClick={handleClickA}>
        TOP ARTISTS
      </button>
      <button className="button" id="topT" onClick={handleClickT}>
        TOP TRACKS
      </button>
      <div id="screenshot">
        {token && (
          <div>
            <div className="customized-container">
              <div className="text">
                Hi <span id="username">{profile.display_name}</span>!
              </div>
            ...
            <div className="selection-text">
              {!clickTopArtists && !clickTopTracks && (
                <h5>
                  Select Top Artists / Tracks to view your stats for the month
                </h5>
              )}
            </div>
          </div>
      ...
      {(topTracks.items || topArtists.items) && (
        <button className="button" onClick={downloadImg}>
          Save Image
        </button>
      )}
